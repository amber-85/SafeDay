...
export const getContacts = async () => {
  const res = await fetch(`${BASE_URL}/contacts`, { headers: getHeaders(false) });
  return res.json();
};
...
export const addElder = async (data: { elder_phone: string; elder_name?: string; relationship: string }) => {
  const res = await fetch(`${BASE_URL}/elders`, {
    method: "POST",
    headers: getHeaders(),
    body: JSON.stringify(data),
  });
...
export const getElders = async () => {
  const res = await fetch(`${BASE_URL}/elders`, { headers: getHeaders(false) });
  return res.json();
};